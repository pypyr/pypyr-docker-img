...
This pypyr-go-terra-aws-k8s image adds the golang runtime, development tools and
...
```bash
$ docker pull pypyr/pypyr-go-terra-aws-k8s
```
...
```bash
$ docker run pypyr/pypyr-go-terra-aws-k8s echo "Ceci n'est pas une pipe"
```
...
```bash
$ docker run pypyr/pypyr-go-terra-aws-k8s magritte
```
...
$ docker run -v ${PWD}:/go/src/github.com/myrepo/myproj -w /go/src/github.com//myrepo/myproj pypyr/pypyr-go-terra-aws-k8s pipelinenamehere`

# mount your host's current dir to the container's working dir.
# Your host has ./pipelines/mypipelinename.yaml
$ docker run -v ${PWD}:/src pypyr/pypyr-go-terra-aws-k8s mypipelinename

# run container's /src/pipelines/mypipelinename.yaml with DEBUG logging level.
$ docker run pypyr/pypyr-go-terra-aws-k8s mypipelinename --log 10

# run container /src/cpipelines/mypipelinename.yaml with INFO logging level.
$ docker run pypyr/pypyr-go-terra-aws-k8s mypipelinename --log 20

# If you don't specify --log it defaults to 20 - INFO logging level.
$ docker run pypyr/pypyr-go-terra-aws-k8s mypipelinename
...
# pipeline yaml.
$ docker run pypyr/pypyr-go-terra-aws-k8s mypipelinename "mykey=value"
```
...
```bash
$ docker run pypyr/pypyr-go-terra-aws-k8s -h
```
...
$ docker run -v ${PWD}:/src pypyr/pypyr-go-terra-aws-k8s simple
...
$ docker run -v ${PWD}:/src pypyr/pypyr-go-terra-aws-k8s substitutions "key1=this is key1 in context,key2=pipe"

# one more time, with extra logging. . .
$ docker run -v ${PWD}:/src pypyr/pypyr-go-terra-aws-k8s substitutions "key1=this is key1 in context,key2=pipe" --log 10
```
...
### Use pypyr/pypyr-go-terra-aws-k8s as a parent image
